...
const (
	...
	defaultUploadMaxSizeBytes = int64(20 * 1024 * 1024)
	defaultUploadExtensions   = "jpg,jpeg,png,webp"
	defaultUploadMIMETypes    = "image/jpeg,image/png,image/webp"
	defaultUploadMinWidth     = 1
	defaultUploadMaxWidth     = 10000
	defaultUploadMinHeight    = 1
	defaultUploadMaxHeight    = 10000
	defaultObjectKeyPrefix    = "images"
)
...
func Load() error {
	loadOnce.Do(func() {
		...
		cfg := &Config{
			...
			Upload: UploadConfig{
				MaxSizeBytes:      v.GetInt64("UPLOAD_MAX_SIZE_BYTES"),
				AllowedExtensions: parseCSV(v.GetString("UPLOAD_ALLOWED_EXTENSIONS")),
				AllowedMIMETypes:  parseCSV(v.GetString("UPLOAD_ALLOWED_MIME_TYPES")),
				MinWidth:          v.GetInt("UPLOAD_MIN_WIDTH"),
				MaxWidth:          v.GetInt("UPLOAD_MAX_WIDTH"),
				MinHeight:         v.GetInt("UPLOAD_MIN_HEIGHT"),
				MaxHeight:         v.GetInt("UPLOAD_MAX_HEIGHT"),
				ObjectKeyPrefix:   strings.TrimSpace(v.GetString("UPLOAD_OBJECT_KEY_PREFIX")),
			},
		}
...
func newViper() *viper.Viper {
	...
	v.SetDefault("UPLOAD_MAX_SIZE_BYTES", defaultUploadMaxSizeBytes)
	v.SetDefault("UPLOAD_ALLOWED_EXTENSIONS", defaultUploadExtensions)
	v.SetDefault("UPLOAD_ALLOWED_MIME_TYPES", defaultUploadMIMETypes)
	v.SetDefault("UPLOAD_MIN_WIDTH", defaultUploadMinWidth)
	v.SetDefault("UPLOAD_MAX_WIDTH", defaultUploadMaxWidth)
	v.SetDefault("UPLOAD_MIN_HEIGHT", defaultUploadMinHeight)
	v.SetDefault("UPLOAD_MAX_HEIGHT", defaultUploadMaxHeight)
	v.SetDefault("UPLOAD_OBJECT_KEY_PREFIX", defaultObjectKeyPrefix)
...
func validateUploadConfig(cfg UploadConfig) error {
	if cfg.MaxSizeBytes <= 0 {
		return fmt.Errorf("configuration error: UPLOAD_MAX_SIZE_BYTES must be greater than 0")
	}

	if len(cfg.AllowedExtensions) == 0 {
		return fmt.Errorf("configuration error: UPLOAD_ALLOWED_EXTENSIONS must not be empty")
	}

	if len(cfg.AllowedMIMETypes) == 0 {
		return fmt.Errorf("configuration error: UPLOAD_ALLOWED_MIME_TYPES must not be empty")
	}

	if cfg.MinWidth < 1 {
		return fmt.Errorf("configuration error: UPLOAD_MIN_WIDTH must be at least 1")
	}

	if cfg.MinHeight < 1 {
		return fmt.Errorf("configuration error: UPLOAD_MIN_HEIGHT must be at least 1")
	}

	if cfg.MaxWidth < cfg.MinWidth {
		return fmt.Errorf("configuration error: UPLOAD_MAX_WIDTH must be greater than or equal to UPLOAD_MIN_WIDTH")
	}

	if cfg.MaxHeight < cfg.MinHeight {
		return fmt.Errorf("configuration error: UPLOAD_MAX_HEIGHT must be greater than or equal to UPLOAD_MIN_HEIGHT")
	}

	if strings.TrimSpace(cfg.ObjectKeyPrefix) == "" {
		return fmt.Errorf("configuration error: UPLOAD_OBJECT_KEY_PREFIX is required")
	}
